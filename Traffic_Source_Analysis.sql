...
/*
1.	Gsearch seems to be the biggest driver of our business. So I will pull monthly 
trends for gsearch sessions and orders so that we can see the growth there 
*/ 
...
/*
2.	A similar monthly trend for Gsearch and splitting out nonbrand and brand campaigns separately. USE COUNT(CASE WHEN..) similar to pivot method
*/ 
...
/*
3.	Dive into nonbrand on Gsearch, and pull monthly sessions and orders split by device type so that we can know better about our traffic sources
*/ 
...
/*
4.	Pull monthly trends for Gsearch, alongside monthly trends for each of our other channels, so that we can know the % of traffic from Gsearch. 
*/ 
...
-- Then, monthly trends for Gsearch, alongside monthly trends for each of our other channels
SELECT
	YEAR(website_sessions.created_at) AS yr, 
    MONTH(website_sessions.created_at) AS mo, 
...
/*
5.	In order to see our website performance improvements over the course of the first 8 months, I will pull session to order conversion rates, by month
...
FROM website_pageviews 
	INNER JOIN website_sessions 
		...
		AND website_sessions.created_at < '2012-07-28' -- prescribed by the stakeholder
...
Use the same time period analyzed last time (Jun 19 – Jul 28).
*/ 
...
/*
8.	In order to quantify the impact of our billing test, analyze the lift generated 
...
WHERE website_pageviews.created_at > '2012-09-10' -- prescribed by the stakeholder
	AND website_pageviews.created_at < '2012-11-10' -- prescribed by the stakeholder
